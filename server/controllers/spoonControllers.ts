...
import {filteredRecipe} from '../types'

export const spoonIngredientController = (req: Request, res: Response, next: NextFunction) => {
      ...
      .then((recipes) => {
        ...
        const recipeRequests = [];
          ...
          recipes.forEach((recipe) => {
            ...
            const missingIngredients = recipe.missedIngredients.map((ingredientObj) => ingredientObj.originalName);
            // store all ingredients, including missing ingredients. 
            const allIngredients = ingredients.concat(missingIngredients);
            ...
            return Promise.all(recipeRequests).then((responses) => {
              // after all promises from recipe fetches have fulfilled, send the result back to the client.
          ...
          function fetchRecipes(recipeId, missingIngredients, allIngredients) {
            ...
            return fetch(`https://api.spoonacular.com/recipes/${recipeId}/information?includeNutrition=false&apiKey=c7d56ffc1c7b44268cb5e992365c6f35`)
              ...
              .then(recipeInfo => {
                ...
                const { id, sourceUrl, readyInMinutes, image, servings, title } = recipeInfo;
                // push the full list of recipe properties to filtered recipes, including passed-in ingredients.
